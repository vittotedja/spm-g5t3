...
const SkillsMapComponent: React.FC<SkillsMapProps> = ({
	...
	//wait for changes to happen to listingData before calling setRoleId

	useEffect(() => {
		async function fetchSkillMatchData() {
			try {
				if (listingData && listingData.role_id) {
					const roleId = listingData.role_id;
					setRoleId(roleId);
					await setInitial(
						setskillMatchData,
			...
			<section className="px-8 py-6">
				{loading ? (
					...
					<div className="max-w-4xl mx-auto">
						...
						<ProgressBar
							percentage={skillMatchData.match_percentage}
						/>
						<div>
