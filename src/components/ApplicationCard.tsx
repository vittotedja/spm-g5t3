...
const ApplicationCard: React.FC<ApplicationCardProps> = ({ application }) => {
  ...
  const roleCardButton = () => {navigate(`/listing-detail/${application.listing.listing_id}`)}
    useEffect(() => {
      setInitial(setSkill, `api/staff_role_skill?staff_id=${application.staff_id}&role_id=${application.listing?.role_id}`)
      setInitial(setRole, `api/role?role_id=${application.listing.role_id}`, false)
  }, []);
      ...
      <div className="rounded-lg shadow-md ring-2 ring-outset ring-emerald-900/20 p-5 w-1/3 mb-4 mr-4 text-left" onClick={roleCardButton}>
        <div className="text-left">
          <div className="flex">
            ...
            <p className={`w-1/2 text-right text-medium ${statusColorMap[application.application_status]}`}>{application.application_status}</p>
          </div>
          ...
          {skill && <ProgressBar
              percentage={skill.match_percentage}
          />}
