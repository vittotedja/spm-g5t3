...
async def get_staff_role(
    ...
    if userid:
        ...
        roles_with_date = [
            role
            for role in all_roles_response.data or []
            if role["appl_close_date"] is not None
            and datetime.fromisoformat(role["appl_close_date"]) >= today
            and role["role_id"] not in applied_role_IDs
        ]
        roles_without_date = [
            role
            for role in all_roles_response.data or []
            if role["appl_close_date"] is None
            and role["role_id"] not in applied_role_IDs
        ]
        ...
        if sort_field == "appl_close_date":
            ...
            if sort_field in ["created_at", "application_date"]:
                all_unapplied_roles.sort(
                    key=lambda x: parse_datetime(
                        x.get(sort_field, "") or "1900-01-01T00:00:00+00:00"
                    )
                    if x.get(sort_field)
                    else datetime.min,
                    reverse=(order == "desc"),
                ...
                all_unapplied_roles.sort(
                    key=lambda x: x.get(sort_field, ""), reverse=(order == "desc")
                )
